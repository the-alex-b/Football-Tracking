...
class TwoPix2PixModel:
    ...
    def initialize(self, opt):
        ...
        # This variable determines where to look for the trained network files
        self.save_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)),"trained_networks")
        ...
        if opt.continue_train:
            self.load_network(self.segmentation_GAN.netG,
                              'G', opt.which_epoch, 'seg')
            self.load_network(self.detection_GAN, 'G',
                              opt.which_epoch, 'detec')
            if self.isTrain:
                self.load_network(self.segmentation_GAN.netD,
                                  'D', opt.which_epoch, 'seg')
                self.load_network(self.detect_GAN.netD, 'D',
                                  opt.which_epoch, 'detec')
    ...
    def backward_D(self):
        if self.isJointTrain:
            ...
            # 1. fake
            fake_AB = seg_GAN.fake_AB_pool.query(
                torch.cat((self.real_A, self.fake_B), 1).data)
            pred_fake = seg_GAN.netD(fake_AB.detach())
            self.segmentation_GAN.loss_D_fake = seg_GAN.criterionGAN(
                pred_fake, False)
            # 2. feal
            real_AB = torch.cat((self.real_A, self.real_B), 1)
            pred_real = seg_GAN.netD(real_AB)
            self.segmentation_GAN.loss_D_real = seg_GAN.criterionGAN(
                pred_real, True)

            # detection network
            detect_GAN = self.detection_GAN
            fake_CD = detect_GAN.fake_AB_pool.query(
                torch.cat((self.real_C, self.fake_D), 1).data)
            pred_fake = detect_GAN.netD(fake_CD.detach())
            self.detection_GAN.loss_D_fake = detect_GAN.criterionGAN(
                pred_fake, False)

            real_CD = torch.cat((self.real_C, self.real_D), 1)
            pred_real = detect_GAN.netD(real_CD)
            self.detection_GAN.loss_D_real = detect_GAN.criterionGAN(
                pred_real, True)
    ...
    def backward_G(self):
        if self.isJointTrain:
            ...
            self.segmentation_GAN.loss_G_GAN = seg_GAN.criterionGAN(
                pred_fake, True)

            # Second, G(A) = B
            self.segmentation_GAN.loss_G_L1 = seg_GAN.criterionL1(
                self.fake_B, self.real_B) * seg_GAN.opt.lambda_A

            detect_GAN = self.detection_GAN
            # Third, G(fakeC) should fake discriminator
            fake_CD = torch.cat((self.real_C, self.fake_D), 1)
            pred_fake = detect_GAN.netD(fake_CD)
            self.detection_GAN.loss_G_GAN = detect_GAN.criterionGAN(
                pred_fake, True)

            # Fourth, G(fakeC) = D
            self.detection_GAN.loss_G_L1 = detect_GAN.criterionL1(
                self.fake_D, self.real_D) * detect_GAN.opt.lambda_A

            self.loss_G = self.segmentation_GAN.loss_G_GAN + self.segmentation_GAN.loss_G_L1 + \
                self.detection_GAN.loss_G_GAN + self.detection_GAN.loss_G_L1
    ...
    def load_network(self, network, network_label, epoch_label, phase_label):
        save_filename = '%s_%s_net_%s.pth' % (
            phase_label, epoch_label, network_label)
