...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    refined_rois = tf.compat.v1.to_int32(tf.math.rint(refined_rois))
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            ...
            arr = class_mask.astype(float)#, (gt_h, gt_w),interp='nearest'
            placeholder[gt_y1:gt_y2, gt_x1:gt_x2] = \
                np.round(np.array(Image.fromarray(arr).resize((gt_w, gt_h),resample=PIL.Image.NEAREST))/255.0).astype(bool)
        ...
        mask = np.array(Image.fromarray(m.astype(float))).resize(config.MASK_SHAPE,resample=PIL.Image.NEAREST) / 255.0
